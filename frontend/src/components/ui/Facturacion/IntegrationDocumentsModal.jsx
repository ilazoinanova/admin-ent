...
import { X, FileText, Loader2, Download, CheckSquare } from "lucide-react";
...
const docKey = (doc) =>
  `${doc.project_id}|${doc.system_integration_name}|${doc.ot_number}|${doc.date}|${doc.report_type}`;

export default function IntegrationDocumentsModal({
  tenantId, departmentId, periodFrom, periodTo,
  currentQty, onConfirm, onClose,
}) {
  ...
  // Set de keys de documentos EXCLUIDOS (todos incluidos por defecto)
  ...
  const selectedCount = docs.length - excluded.size;

  const hasChanges = useMemo(
    () => selectedCount !== currentQty,
    [selectedCount, currentQty],
  );
  ...
  const handleExcel = () => {
    const rows = docs.map((doc, i) => ({
      ...
      [t("included")]:   excluded.has(docKey(doc)) ? t("no") : t("yes"),
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, t("integrationDocsTitle"));
    XLSX.writeFile(wb, `documentos-integracion-${periodFrom}-${periodTo}.xlsx`);
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl border dark:border-gray-700 flex flex-col w-full max-w-4xl max-h-[85vh]">
        ...
        <div className="bg-[#0b1b3b] text-white px-5 py-4 rounded-t-xl flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold">{t("integrationDocsTitle")}</p>
              <p className="text-xs text-white/50 mt-0.5">
                {fmtPeriodDate(periodFrom)} → {fmtPeriodDate(periodTo)}
              </p>
            </div>
          </div>
                  ...
                  <thead className="sticky top-0 bg-gray-50 dark:bg-gray-700/80 border-b border-gray-200 dark:border-gray-600">
                    <tr>
                      <th className="text-center px-3 py-3 w-10">
                        <CheckSquare size={13} className="mx-auto text-gray-400 dark:text-gray-500" />
                      </th>
                      ...
                      return (
                        <tr
                          ...
                          className={`cursor-pointer transition ${
                            isExcluded
                              ? "opacity-40 bg-red-50/60 dark:bg-red-900/10"
                        ...
                        >
                          <td className="px-3 py-2.5 text-center">
                            <input
                              ...
                              readOnly
                              checked={!isExcluded}
                              className="pointer-events-none accent-emerald-600 w-3.5 h-3.5"
                            />
                          </td>
              ...
              <div className="shrink-0 border-t border-gray-200 dark:border-gray-700 px-5 py-3 flex items-center justify-between bg-gray-50 dark:bg-gray-800/60">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm font-bold text-gray-800 dark:text-gray-100">
                    {selectedCount} / {docs.length} {t("documentsProcessed")}
                  </span>
                  {onConfirm && (
                    <button
                      type="button"
                      ...
                      disabled={!hasChanges}
                      className="flex items-center gap-1.5 text-xs bg-[#0b1b3b] hover:bg-[#162d5e] disabled:opacity-40 disabled:cursor-not-allowed text-white px-3 py-1.5 rounded-lg transition"
                    >
                      {t("applySelection")}
                    </button>
                  )}
